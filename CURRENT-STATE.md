...
- [x] AUTH-C local (0C-AUTH-C) — Durable `identity.auth.security` v1 outbox events (safe metadata; unknown-account failures omit identifiers); HumanChallenge production adapter is Cloudflare Turnstile (stdlib Siteverify; production widget credentials not committed; `fake` rejected in staging/production); email uses Amazon SES when `NOTIFICATIONS_EMAIL_MODE=external` and `EMAIL_PROVIDER=ses`; SMS uses Netgsm when `NOTIFICATIONS_SMS_MODE=external` and `SMS_PROVIDER=netgsm` (`disabled` never marked sent; `external` requires adapter). Load/abuse/provider-failure proofs in Identity tests. Classification: **HUMAN_CHALLENGE_ADAPTER_READY_CREDENTIALS_PENDING**; email **PROVIDER-B READY_FOR_REVIEW**; SMS **PROVIDER-C READY_FOR_REVIEW** (`LIVE_NETGSM_TEST_PENDING`). Runbook: `docs/operations/AUTH-SECURITY.md`.
...
3. AUTH-C HumanChallenge production adapter is Cloudflare Turnstile (stdlib Siteverify). Production widget credentials, frontend widget, and live SES/Netgsm credentials remain launch blockers. Do not call full 0C-AUTH production-ready until those are onboarded.
