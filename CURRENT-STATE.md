...
**Phase 0B** — backend scaffold in progress (0B-01 through 0B-108 complete as listed above). NOTIFY-A catalog/resolver, approved `000052`, stores, HTTP, and selected AUTH-C materialization are **READY_FOR_REVIEW** (providers, push endpoints, and domain producers remain later).
...
4. NOTIFY-A application layer is at **NOTIFY_A_APPLICATION_READY_FOR_REVIEW**. Preference/consent/inbox HTTP and selected AUTH-C materialization exist. Do not treat this as provider-ready or legally reviewed. Push registration, email/SMS vendor dispatch, and marketplace producers remain NOTIFY-B.
