...
- [x] NOTIFY-D local (0C-NOTIFY-D) — Production push transports: Web Push (VAPID / RFC 8291), FCM HTTP v1 `messages:send`, APNs HTTP/2 token auth. Provider-neutral NOTIFY-C ports preserved. JIT decrypt; no adapter retry; invalid endpoints revoked only on authoritative provider outcomes. 000053 unchanged. No new migration. SES/Netgsm/Turnstile/Identity/Staff/Trust/EİDS/ADR-015 unchanged. Frontend `WEB_PUSH_FRONTEND_PENDING`. Mobile `MOBILE_PUSH_CLIENT_PENDING`. Live: `LIVE_WEBPUSH_TEST_PENDING`, `LIVE_FCM_TEST_PENDING`, `LIVE_APNS_TEST_PENDING`.

---

## What Is In Progress

**Phase 0B** — backend scaffold in progress (0B-01 through 0B-108 complete as listed above). NOTIFY-A remains frozen (`000052`). NOTIFY-B producers + dispatcher are **READY_FOR_REVIEW**. NOTIFY-C push endpoint registry is **READY_FOR_REVIEW** (`000053`). NOTIFY-D Web Push/FCM/APNs transports are **READY_FOR_REVIEW** (live credentials pending). PROVIDER-B SES email is **READY_FOR_REVIEW**. PROVIDER-C Netgsm SMS is **READY_FOR_REVIEW** (`LIVE_NETGSM_TEST_PENDING`). ADR-015 TR Compliance Gateway / data residency is **READY_FOR_REVIEW** (documentation freeze only; no gateway implementation).
...
4. NOTIFY-D application layer is at **NOTIFY_D_APPLICATION_READY_FOR_REVIEW**. Web Push, FCM HTTP v1, and APNs token transports exist. Live provider credentials, consumer Web Push registration (`WEB_PUSH_FRONTEND_PENDING`), and mobile clients (`MOBILE_PUSH_CLIENT_PENDING`) remain. Do not treat this as legally reviewed. Saved-search match and moderation warning cutover remain later.
