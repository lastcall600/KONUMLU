...
- [x] WEB-PUSH-CLIENT local — Consumer browser Web Push: `public/sw.js`, user-gesture permission on `/bildirimler`, PushManager subscribe + `POST /v1/push-endpoints`, opaque endpoint id only in app storage, disable/revoke current endpoint, logout does not revoke (NOTIFY-C). Public VAPID: `NEXT_PUBLIC_WEBPUSH_VAPID_PUBLIC_KEY`. No migration. FCM/APNs mobile client untouched. Live send: `LIVE_WEBPUSH_TEST_PENDING`.

---

## What Is In Progress

**Phase 0B** — backend scaffold in progress (0B-01 through 0B-108 complete as listed above). NOTIFY-A remains frozen (`000052`). NOTIFY-B producers + dispatcher are **READY_FOR_REVIEW**. NOTIFY-C push endpoint registry is **READY_FOR_REVIEW** (`000053`). NOTIFY-D Web Push/FCM/APNs transports are **READY_FOR_REVIEW** (live credentials pending). Consumer Web Push client is **READY_FOR_REVIEW**. PROVIDER-B SES email is **READY_FOR_REVIEW**. PROVIDER-C Netgsm SMS is **READY_FOR_REVIEW** (`LIVE_NETGSM_TEST_PENDING`). ADR-015 TR Compliance Gateway / data residency is **READY_FOR_REVIEW** (documentation freeze only; no gateway implementation).
