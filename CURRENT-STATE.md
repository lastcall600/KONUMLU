...
- [x] ADR-015 — 🔒 FROZEN / Accepted: Türkiye Compliance Gateway and data residency boundary (main platform remains Germany/Hetzner; TR node is not a general app server). 0C-EIDS-A implements the signed-decision protocol without rewriting this ADR.
...
- [x] AUTH-A local (0C-AUTH-A) — Identity abuse primitives: multi-dimensional Valkey rate limits on public auth (including previously uncovered signup complete, reset verify/complete, passkey enrollment), hashed subjects, fail-closed abuse checks, internal `RiskDecision`, HumanChallenge port (`fake`/`unconfigured` plus production Cloudflare Turnstile adapter). Trust untouched. See `docs/architecture/auth-abuse.md`.
...
- [x] AUTH-C local (0C-AUTH-C) — Durable `identity.auth.security` v1 outbox events (safe metadata; unknown-account failures omit identifiers); HumanChallenge production adapter is Cloudflare Turnstile (stdlib Siteverify; production widget credentials not committed; `fake` rejected in staging/production); consumer Turnstile widget uses public sitekey only (`NEXT_PUBLIC_TURNSTILE_SITEKEY`, explicit render, memory-only `challengeToken`); email uses Amazon SES when `NOTIFICATIONS_EMAIL_MODE=external` and `EMAIL_PROVIDER=ses`; SMS uses Netgsm when `NOTIFICATIONS_SMS_MODE=external` and `SMS_PROVIDER=netgsm` (`disabled` never marked sent; `external` requires adapter). Load/abuse/provider-failure proofs in Identity tests. Classification: **HUMAN_CHALLENGE_ADAPTER_READY_CREDENTIALS_PENDING**; consumer widget **READY_FOR_REVIEW** (`LIVE_TURNSTILE_E2E_PENDING`); email **PROVIDER-B READY_FOR_REVIEW**; SMS **PROVIDER-C READY_FOR_REVIEW** (`LIVE_NETGSM_TEST_PENDING`). Runbook: `docs/operations/AUTH-SECURITY.md`.
...
- [x] PROVIDER-B local — Amazon SES transactional email adapter (`internal/infrastructure/email/ses`; SES API v2 `SendEmail`; AWS SDK v2; default credential chain; no SMTP). Wires Identity verification `EmailSender` and Notifications `ChannelSender`. Dispatcher remains retry authority. SendEmail accept ≠ delivered. No migration. SMS/Push/Turnstile/Staff/Trust/EİDS/ADR-015 unchanged.
- [x] PROVIDER-C local — Netgsm transactional SMS + Identity OTP adapter (`internal/infrastructure/sms/netgsm`; REST v2 `POST /sms/rest/v2/send` and `POST /sms/rest/v2/otp`; HTTP Basic Auth; stdlib `net/http`; no SDK). Server-owned `msgheader`. Opaque string `jobid` on `ProviderRef`. Accept ≠ handset delivery. No adapter retry loop; dispatcher/outbox remain retry authority. Ambiguous timeouts are at-least-once. No `iysfilter`/marketing policy. No migration. No new dependency. SES/Push/Turnstile/Staff/Trust/EİDS/ADR-015 unchanged. Live send: `LIVE_NETGSM_TEST_PENDING`.
- [x] NOTIFY-C local (0C-NOTIFY-C) — Durable `notifications.push_endpoints` (`000053_notifications_push_endpoints`; additive). User-owned web/Android/iOS endpoint registry with AES-256-GCM encrypted provider material, HMAC-SHA256 uniqueness over endpoint URL (web) or opaque token (mobile), register/list/revoke HTTP (session CSRF/Origin). `endpoint_key_id` is stable `v1`; encryption-key rotation is not implemented. Logout does **not** revoke endpoints. Registration is not delivery: unconfigured FCM/APNs/WebPush senders never fake-accept. 000052 unchanged. SES/Netgsm/Turnstile/Identity/Staff/Trust/EİDS/ADR-015 unchanged.
- [x] NOTIFY-D local (0C-NOTIFY-D) — Production push transports: Web Push (VAPID / RFC 8291), FCM HTTP v1 `messages:send`, APNs HTTP/2 token auth. Provider-neutral NOTIFY-C ports preserved. JIT decrypt; no adapter retry; invalid endpoints revoked only on authoritative provider outcomes. 000053 unchanged. No new migration. SES/Netgsm/Turnstile/Identity/Staff/Trust/EİDS/ADR-015 unchanged. Mobile `MOBILE_PUSH_CLIENT_PENDING`. Live: `LIVE_WEBPUSH_TEST_PENDING`, `LIVE_FCM_TEST_PENDING`, `LIVE_APNS_TEST_PENDING`.
- [x] 0C-EIDS-A local — Türkiye→Germany signed verification decision protocol (Ed25519, opaque `subject_ref`, PostgreSQL replay `000054`, internal ingress Bearer). Same-attempt apply is monotonic on signed `issued_at` (stale envelopes are recorded, not applied). Official EİDS/e-Devlet adapter not implemented. Trust/Staff/Turnstile/Notifications untouched. Classification: **READY_FOR_REVIEW**.

---

## What Is In Progress

**Phase 0B** — backend scaffold in progress (0B-01 through 0B-108 complete as listed above). NOTIFY-A remains frozen (`000052`). NOTIFY-B producers + dispatcher are **READY_FOR_REVIEW**. NOTIFY-C push endpoint registry is **READY_FOR_REVIEW** (`000053`). NOTIFY-D Web Push/FCM/APNs transports are **READY_FOR_REVIEW** (live credentials pending). Consumer Web Push client is **READY_FOR_REVIEW**. PROVIDER-B SES email is **READY_FOR_REVIEW**. PROVIDER-C Netgsm SMS is **READY_FOR_REVIEW** (`LIVE_NETGSM_TEST_PENDING`). ADR-015 TR Compliance Gateway / data residency is **READY_FOR_REVIEW** (🔒 FROZEN). 0C-EIDS-A signed decision protocol is **READY_FOR_REVIEW** (`000054`; official adapter not implemented; same-attempt apply is monotonic on signed `issued_at`).
...
3. AUTH-C HumanChallenge production adapter is Cloudflare Turnstile (stdlib Siteverify). Consumer widget is implemented (public sitekey only). Missing required tokens return `403` `{ "error": "challenge_required" }`; generic `403`/`forbidden` does not activate Turnstile. Production widget credentials and live SES/Netgsm credentials remain launch blockers. Do not call full 0C-AUTH production-ready until those are onboarded.
4. NOTIFY-D application layer is at **NOTIFY_D_APPLICATION_READY_FOR_REVIEW**. Consumer Web Push registration is **READY_FOR_REVIEW**. Live provider credentials and mobile clients (`MOBILE_PUSH_CLIENT_PENDING`) remain. Do not treat this as legally reviewed. Saved-search match and moderation warning cutover remain later.
