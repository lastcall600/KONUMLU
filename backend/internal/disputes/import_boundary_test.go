...
func TestDisputesProductionDoesNotImportPeerImplementations(t *testing.T) {
	assertNoPeerImpl(t, ".")
	...
	for _, e := range entries {
		...
		for _, imp := range f.Imports {
			...
			for _, peer := range []string{
				...
				"messaging", "notifications", "transactions", "payments", "deliveries",
				"verified", "reviews", "moderation", "staffauth",
			} {
