...
func TestPaymentsProductionDoesNotImportPeerImplementations(t *testing.T) {
	assertNoPeerImpl(t, ".")
...
func assertNoPeerImpl(t *testing.T, dir string) {
	...
	for _, e := range entries {
		...
		for _, imp := range f.Imports {
			...
			for _, peer := range []string{"identity", "needs", "offers", "businesses", "listings", "trust", "messaging", "notifications", "transactions"} {
