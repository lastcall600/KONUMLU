...
func TestSavedSearchProductionImportsOnlyContracts(t *testing.T) {
	...
	for _, e := range entries {
		...
		for _, imp := range f.Imports {
			...
			if p == "backend/internal/search" || strings.HasPrefix(p, "backend/internal/search/") {
				t.Fatalf("%s imports search %s", name, p)
			}
