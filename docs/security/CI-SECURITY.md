...
These two third-party IDs are listed in `.github/security/govulncheck-known.txt`. 0C-07A moved the backend toolchain to Go 1.26.8 (supported patched line; first 1.26 release that includes all previously reported stdlib HIGH fixes is 1.26.6). New third-party IDs / new fixed CRITICAL image CVEs fail CI.

npm audit (consumer + admin lockfiles): 0 vulnerabilities at high/critical.

Container Alpine 3.21 OS layer: 0 CRITICAL/HIGH (fixed). Gobinary HIGH remaining after 0C-07A are third-party (`golang.org/x/crypto`, `golang.org/x/text`); those require a separate authorized module upgrade, not a toolchain bump.
