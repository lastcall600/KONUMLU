...
| `auth-abuse.md` | AUTH-A Identity abuse primitives: multi-dimensional Valkey rate limits, RiskDecision, HumanChallenge port (Cloudflare Turnstile production adapter; `fake` is dev/test only). AUTH-C security events. Email: Amazon SES transactional adapter. SMS remains a launch blocker. |
...
| `/backend/internal/infrastructure/` | Provider adapter implementations (storage, notifications, maps, payments, EİDS, HumanChallenge, email) | `humanchallenge/turnstile` Cloudflare Turnstile Siteverify (stdlib `net/http`; no SDK; no `remoteip` in V1); `email/ses` Amazon SES API v2 `SendEmail` (AWS SDK v2; default credential chain; no SMTP; transport only); `notifications` email/SMS bind (`disabled`/`external`; SES registered when `EMAIL_PROVIDER=ses`); `storage` S3-compatible adapter (AWS SDK v2; signed PUT/GET; optional public base URL; custom endpoint/path-style; no production vendor/CDN); `eids` unconfigured gateway (unavailable, never verified; no official SDK) |
