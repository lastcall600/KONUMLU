...
| `auth-abuse.md` | AUTH-A Identity abuse primitives: multi-dimensional Valkey rate limits, RiskDecision, HumanChallenge port (Cloudflare Turnstile production adapter; `fake` is dev/test only). AUTH-C security events + remaining email/SMS launch blockers. |
...
| `/backend/internal/infrastructure/` | Provider adapter implementations (storage, notifications, maps, payments, EİDS, HumanChallenge) | `humanchallenge/turnstile` Cloudflare Turnstile Siteverify (stdlib `net/http`; no SDK; no `remoteip` in V1); `notifications` email/SMS adapters (no vendor SDK; `disabled`/`external` modes); `storage` S3-compatible adapter (AWS SDK v2; signed PUT/GET; optional public base URL; custom endpoint/path-style; no production vendor/CDN); `eids` unconfigured gateway (unavailable, never verified; no official SDK) |
