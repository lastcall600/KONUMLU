...
| `notifications-policy.md` | NOTIFY-A catalog/consent/inbox plus NOTIFY-B marketplace producers, dispatcher claim/retry, NOTIFY-C encrypted push endpoints (`000053`; transport later). |
...
| `/backend/internal/infrastructure/` | Provider adapter implementations (storage, notifications, maps, payments, EİDS, HumanChallenge, email, SMS) | `humanchallenge/turnstile` Cloudflare Turnstile Siteverify (stdlib `net/http`; no SDK; no `remoteip` in V1); `email/ses` Amazon SES API v2 `SendEmail` (AWS SDK v2; default credential chain; no SMTP; transport only); `sms/netgsm` Netgsm REST v2 `/send` + `/otp` (stdlib `net/http`; HTTP Basic Auth; no SDK; transport only; `jobid` opaque string); `notifications` email/SMS bind (`disabled`/`external`; SES when `EMAIL_PROVIDER=ses`; Netgsm when `SMS_PROVIDER=netgsm`); `storage` S3-compatible adapter (AWS SDK v2; signed PUT/GET; optional public base URL; custom endpoint/path-style; no production vendor/CDN); `eids` unconfigured gateway (unavailable, never verified; no official SDK) |
