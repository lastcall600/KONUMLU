...
| `auth-abuse.md` | AUTH-A Identity abuse primitives: multi-dimensional Valkey rate limits, RiskDecision, HumanChallenge port (Cloudflare Turnstile production adapter; `fake` is dev/test only). AUTH-C security events. Email: Amazon SES transactional adapter. SMS: Netgsm REST v2 transactional + OTP (transport only). |
...
| `notifications-policy.md` | NOTIFY-A catalog/consent/inbox plus NOTIFY-B marketplace producers, dispatcher claim/retry, NOTIFY-C encrypted push endpoints (`000053`), NOTIFY-D Web Push/FCM/APNs transports. |
| `eids-tr-decision-protocol.md` | 0C-EIDS-A TR→DE signed verification decision (Ed25519, subject_ref, replay). ADR-015 boundary. Official provider not included. |
